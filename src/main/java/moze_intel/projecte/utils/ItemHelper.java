...
public final class ItemHelper
{
	...
	public static boolean areItemStacksEqualIgnoreNBT(ItemStack stack1, ItemStack stack2)
	{
		...
		// MITE separates item variants (subtype) from durability loss
		// (damage); the "same item" comparison must use the subtype.
		if (stack1.getItemSubtype() == OreDictionary.WILDCARD_VALUE || stack2.getItemSubtype() == OreDictionary.WILDCARD_VALUE)
		{
			return true;
		}

		return stack1.getItemSubtype() == stack2.getItemSubtype();
	}

	public static boolean basicAreStacksEqual(ItemStack stack1, ItemStack stack2)
	{
		// MITE stores item variants in the subtype field and durability loss
		// in getItemDamage(); two stacks of the same item but different
		// metadata must NOT be treated as the same stack (otherwise the
		// transmutation table would collapse every meta branch into one).
		return (stack1.getItem() == stack2.getItem()) && (stack1.getItemSubtype() == stack2.getItemSubtype());
	}
	...
	public static boolean containsItemStack(List<ItemStack> list, ItemStack toSearch)
	{
		for (ItemStack stack : list) {
			...
			if (stack.getItem().equals(toSearch.getItem())) {
				if (!stack.getHasSubtypes() || stack.getItemSubtype() == toSearch.getItemSubtype()) {
					return true;
				}
			}
		}
	...
	{
		Item item = (Item) moze_intel.projecte.compat.PECompatHelper.getItem(internal);
