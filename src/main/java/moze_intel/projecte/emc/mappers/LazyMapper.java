...
public class LazyMapper implements IEMCMapper<NormalizedSimpleStack, Integer> {
	...
	public void addMappings(IMappingCollector<NormalizedSimpleStack, Integer> mapper, Configuration config) {
		...
		// MITE does not register the "treeSapling"/"treeLeaves" OreDictionary
		// entries, so the OD defaults never reach the items. Set every subtype
		// directly - the 4 tree types (oak/spruce/birch/jungle) each have their
		// own metadata, and only damage 0 used to get a value (leaves 1, sapling 0).
		for (int i = 0; i < 4; i++)
		{
			addMapping(new ItemStack(Blocks.leaves, 1, i), 1);
			addMapping(new ItemStack(Blocks.sapling, 1, i), 32);
		}
	...
	protected void addMapping(ItemStack itemStack, int value) {
		NormalizedSimpleStack stack = NormalizedSimpleStack.getFor(itemStack);
		if (stack != null) {
			this.mapper.setValueBefore(stack, value);
		}
	}
