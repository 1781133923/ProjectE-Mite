...
public abstract class NormalizedSimpleStack {
	...
	public static NormalizedSimpleStack getFor(String itemName, int damage) {
		...
		try
		{
			...
			PELogger.logWarn("Could not create NSSItem: " + e.getMessage());
			return null;
		}
	...
	public static NormalizedSimpleStack getFor(ItemStack stack) {
		...
		return getFor(stack.getItem(), stack.getItemSubtype());
	}
	...
	public static class NSSItem extends NormalizedSimpleStack {
		...
		private NSSItem(String itemName, int damage) {
			net.minecraft.Item item = moze_intel.projecte.compat.PECompatHelper.getItem(itemName);
			if (item == null) {
				throw new IllegalArgumentException("Invalid Item with itemName = " + itemName);
			}
			// Unify the spelling/case with the keys that are generated from
			// ItemStack/Item lookups. GameRegistry builds the key from the
			// mod's namespace + registry name (e.g. "Minecraft:book"), while
			// hand-written JSON files use "minecraft:book". Because equals()
			// is case-sensitive, those used to become two different graph
			// nodes and fixed values never fed into the recipe conversions.
			// Normalizing here makes every source produce the same node.
			String normalizedName = itemName;
			try {
				GameRegistry.UniqueIdentifier identifier = GameRegistry.findUniqueIdentifierFor(item);
				if (identifier != null) {
					normalizedName = identifier.modId + ":" + identifier.name;
				}
			} catch (Exception e) {
				// Item is not registered with GameRegistry: keep the requested name.
			}
			this.itemName = normalizedName;
			this.damage = damage;
		}
		...
		public String toString() {
			Object obj = moze_intel.projecte.compat.PECompatHelper.getItem(itemName);

			if (obj != null) {
				return String.format("%s(%s:%s)", itemName, ((net.minecraft.Item) obj).itemID, damage == OreDictionary.WILDCARD_VALUE ? "*" : damage);
			}
