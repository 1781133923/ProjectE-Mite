	...
	public static void map()
	{
		...
		for (Map.Entry<NormalizedSimpleStack, Integer> entry: graphMapperValues.entrySet()) {
			if (entry.getKey() instanceof NormalizedSimpleStack.NSSItem)
			{
				...
				String targetName = normStackItem.itemName;
				int colon = targetName.indexOf(':');
				String normalizedTarget = targetName.substring(colon + 1).replace("item.", "").replace("tile.", "");
				boolean anyMatched = false;
				// MITE has many distinct items sharing one unlocalized name
				// (material-variant fishing rods, and block ItemBlocks vs the
				// separate item placed by the recipe, e.g. doors). Assign the
				// computed value to every item matching the name, not just the
				// first one, so none of them are left without EMC.
				for (net.minecraft.Item item : net.minecraft.Item.itemsList) {
					if (item == null) {
						continue;
					}
					String itemName = item.getUnlocalizedName();
					if (itemName != null && itemName.replace("item.", "").replace("tile.", "").equalsIgnoreCase(normalizedTarget)) {
						if (item instanceof net.minecraft.ItemCoin) {
							// MITE coins are currency, not transmutable materials:
		...
		// MITE's filled buckets are not registered with Forge's
		// FluidContainerRegistry, so no recipe/fluid mapping ever gives them a
		// value. A filled bucket is worth exactly its empty bucket (the liquid
		// inside has no EMC: water is free, lava/milk never enter the EMC map).
		// This is done after graph generation on purpose: registering
		// "filled = empty" as a graph conversion would form a cycle with
		// MITE's "pour out" crafting recipes (stone bucket -> empty bucket,
		// which subtracts the returned empty bucket) and the solver would
		// resolve every bucket to 0.
		for (net.minecraft.Item item : net.minecraft.Item.itemsList) {
			...
			SimpleStack empty = new SimpleStack(container.itemID, 1, 0);
			Integer emptyValue = emc.get(empty);
			if (emptyValue != null) {
				emc.put(new SimpleStack(item.itemID, 1, 0), emptyValue);
			}
		}
