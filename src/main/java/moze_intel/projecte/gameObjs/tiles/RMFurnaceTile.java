...
public class RMFurnaceTile extends TileEmc implements IInventory, ISidedInventory, IEmcAcceptor
{
			...
			if (furnaceBurnTime == 0 && canSmelt())
			{
				...
				if (furnaceBurnTime > 0)
				{
					...
					if (inventory[0] != null)
					{
						...
						{
							// Only swap in a container item (e.g. bucket from a
							// lava bucket) when the fuel actually has one;
							// fuels without a container item (e.g. the LavaInPipes
							// item) would otherwise crash here with a null
							// ItemStack.
							net.minecraft.Item containerItem = inventory[0].getItem().getContainerItem();
							inventory[0] = containerItem != null ? new ItemStack(containerItem) : null;
						}
					}
				}
			}
	...
	private void pullFromInventories()
	{
		...
		if (tile instanceof ISidedInventory)
		{
			...
			if (slots.length > 0)
			{
				for (int i : slots)
				{
					...
					{
						if (moze_intel.projecte.compat.PECompatHelper.isFuel(stack) || stack.getItem() instanceof IItemEmc)
						{
							if (inventory[0] == null)
							{
								inventory[0] = stack;
		...
		else if (tile instanceof IInventory)
		{
			...
			else if (FurnaceRecipes.smelting().getSmeltingResult(stack, -1) == null)
				{
					continue;
				}
	...
	private void pushToInventories()
	{
		for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS)
		{
			...
			// The four horizontal sides must not auto-eject into hoppers -
			// LavaInPipes changes hopper behaviour and can swallow or convert
			// the output, making it look like smelted items disappear. The
			// bottom may still feed a hopper normally.
			if (dir.offsetY == 0 && tile instanceof net.minecraft.TileEntityHopper)
			{
				continue;
			}
	...
	private boolean canSmelt() 
	{
		...
		ItemStack smeltResult = FurnaceRecipes.smelting().getSmeltingResult(toSmelt, -1);
		if (smeltResult == null) 
		{
			return false;
		}
		...
		if (!(smeltResult.itemID == currentSmelted.itemID && smeltResult.getItemSubtype() == currentSmelted.getItemSubtype()))
		{
			return false;
		}
	...
	{
		int val = moze_intel.projecte.compat.PECompatHelper.getBurnTime(stack);
	...
	public boolean isItemValidForSlot(int slot, ItemStack stack) 
	{
		...
		if (slot == 0)
		{
			return moze_intel.projecte.compat.PECompatHelper.isFuel(stack) || stack.getItem() instanceof IItemEmc;
		}
		else if (slot >= 1 && slot <= 13)
		{
			return FurnaceRecipes.smelting().getSmeltingResult(stack, -1) != null;
		}
