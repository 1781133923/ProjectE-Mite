...
public class EntityFireProjectile extends PEProjectile
{
	...
	{
		if(!worldObj.isRemote && mop.getEntityHit() == null)
		{
			int x = mop.blockX;
			int y = mop.blockY;
			int z = mop.blockZ;
			...
			{
				PlayerHelper.checkedReplaceBlock(((ServerPlayer) getThrower()), x, y, z, Blocks.flowing_lava, 0);
			}
			else
			{
				for(int x1 = x - 1; x1 <= x + 1; x1++)
					for(int y1 = y - 1; y1 <= y + 1; y1++)
						for(int z1 = z - 1; z1 <= z + 1; z1++)
						{
							...
							{
								PlayerHelper.checkedPlaceBlock(((ServerPlayer) getThrower()), x1, y1, z1, Blocks.fire, 0);
							} else if (worldObj.getBlock(x1, y1, z1) == Blocks.sand)
							{
								PlayerHelper.checkedReplaceBlock(((ServerPlayer) getThrower()), x1, y1, z1, Blocks.glass, 0);
							}
