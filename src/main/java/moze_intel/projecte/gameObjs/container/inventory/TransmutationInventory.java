...
public class TransmutationInventory implements IInventory
{
	...
	public void updateOutputs(boolean async)
	{
		...
		if (inventory[LOCK_INDEX] != null)
		{
			...
			while (iter.hasNext())
			{
				...
				if (EMCHelper.getBuyValue(stack) > reqEmc)
				{
					iter.remove();
					continue;
				}
		...
		else
		{
			...
			while (iter.hasNext())
			{
				...
				if (emc < EMCHelper.getBuyValue(stack))
				{
					iter.remove();
					continue;
				}
	...
	public void writeIntoOutputSlot(int slot, ItemStack item)
	{
		...
		if (EMCHelper.doesItemHaveEmc(item) && EMCHelper.getBuyValue(item) <= this.emc && Transmutation.hasKnowledgeForStack(item, player))
		{
			inventory[slot] = item;
		}
		else
