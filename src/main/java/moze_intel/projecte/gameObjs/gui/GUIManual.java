...
public class GUIManual extends GuiScreen
{
	...
	public static void drawItemStackToGui(Minecraft mc, ItemStack item, int x, int y, boolean fixLighting)
	{
		...
		new net.minecraft.RenderItem().renderItemAndEffectIntoGUI(mc.fontRenderer, mc.getTextureManager(), item, x, y);
	...
	private void updateButtons()
	{
		if (isViewingIndex())
		{
			((PageTurnButton) this.buttonList.get(0)).drawButton = true;
			((PageTurnButton) this.buttonList.get(1)).drawButton = currentSpread != 0;
			((TocButton) this.buttonList.get(2)).drawButton = false;
			for (int i = 3; i < this.buttonList.size(); i++)
			{
				...
				((IndexLinkButton) buttonList.get(i)).drawButton = indexLinks.get(((IndexPage) spread.getLeft())).contains(buttonList.get(i))
		...
		{
			((PageTurnButton) this.buttonList.get(0)).drawButton = false;
			((PageTurnButton) this.buttonList.get(1)).drawButton = true;
			((TocButton) this.buttonList.get(2)).drawButton = true;
			for (int i = 3; i < this.buttonList.size(); i++)
			{
				((IndexLinkButton) this.buttonList.get(i)).drawButton = false;
			}
		} else
		{
			((PageTurnButton) this.buttonList.get(0)).drawButton = true;
		...
		} else
		{
			...
			if (page instanceof ItemPage)
			{
				...
				drawItemStackToGui(mc, itemPage.getItemStack(), Math.round(contentX * GUI_SCALE_FACTOR), 22, !(itemPage.getItemStack().getItem() instanceof ItemBlock)
						|| itemPage.getItemStack().getItem() == net.minecraft.Item.getItem(ObjHandler.confuseTorch));
			}
		}
	}
