...
public class ObjHandler
{
	...
	// Armor must be declared first: DMArmor/RMArmor/GemArmorBase take their
	// IdUtil ID in the constructor (like MoreMetals), and they used to consume
	// the first IDs during static init (4256-4267) while every other item got
	// its ID later at register time. Declaring them here keeps that exact
	// ordering so item IDs - and existing worlds - stay unchanged.
	...
	public static void addRecipes()
	{
		...
		//Matter
		GameRegistry.addRecipe(new ItemStack(matter, 1, 0), "AAA", "ADA", "AAA", 'D', Blocks.emerald_block, 'A', new ItemStack(fuels, 1, 2));
		...
		//Condenser (may be disabled dynamically via config/projectEO.cfg)
		...
		//Condenser MK2 (may be disabled dynamically via config/projectEO.cfg)
		...
		//Transmutation Table (may be disabled dynamically via config/projectEO.cfg)
		...
		//TransmutationTablet (may be disabled dynamically via config/projectEO.cfg)
		...
		//Mercurial Eye
		GameRegistry.addRecipe(new ItemStack(mercEye), "OBO", "BRB", "BDB", 'O', Blocks.obsidian, 'B', Blocks.brick_block, 'R', new ItemStack(matter, 1, 0), 'D', Items.diamond);
		...
		// Charcoal -> coal exchange: 4 charcoal + Philosopher's Stone -> 1
		// coal (the Philosopher's Stone is not consumed; 4 x charcoal EMC
		// matches 1 x coal EMC).
		GameRegistry.addShapelessRecipe(new ItemStack(Items.coal), philosStone,
				new ItemStack(Items.coal, 1, 1), new ItemStack(Items.coal, 1, 1),
				new ItemStack(Items.coal, 1, 1), new ItemStack(Items.coal, 1, 1));
  	// Philosopher's stone conversions: 8 gold -> emerald, 2 emerald -> diamond
	// (and their reverses), rebalancing the gem economy.
	GameRegistry.addShapelessRecipe(new ItemStack(Items.emerald), philosStone, Items.gold_ingot, Items.gold_ingot, Items.gold_ingot, Items.gold_ingot, Items.gold_ingot, Items.gold_ingot, Items.gold_ingot, Items.gold_ingot);
	GameRegistry.addShapelessRecipe(new ItemStack(Items.gold_ingot, 8), philosStone, Items.emerald);
	GameRegistry.addShapelessRecipe(new ItemStack(Items.diamond), philosStone, Items.emerald, Items.emerald);
	GameRegistry.addShapelessRecipe(new ItemStack(Items.emerald, 2), philosStone, Items.diamond);
	...
	public static void registerPhiloStoneSmelting()
	{
		...
		for (Entry<Integer, ItemStack> entry : ((java.util.Map<Integer, ItemStack>) FurnaceRecipes.smelting().getSmeltingList()).entrySet())
		{
			if (entry.getKey() == null || entry.getValue() == null || entry.getKey() <= 0 || net.minecraft.Item.itemsList[entry.getKey()] == null)
			{
				continue;
			}
	...
	public static class FuelHandler implements IFuelHandler
	{
		...
		public int getBurnTime(ItemStack fuel)
		{
			...
			} else if (fuel.getItem() == net.minecraft.Item.getItem(fuelBlock))
			{
				switch (fuel.getItemDamage())
				{
					case 0:
