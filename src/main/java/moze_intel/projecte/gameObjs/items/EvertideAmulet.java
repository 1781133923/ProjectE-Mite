...
public class EvertideAmulet extends ItemPE implements IProjectileShooter, IBauble, IPedestalItem, IFluidContainerItem
{
	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int sideHit, float f1, float f2, float f3)
	{
		if (!world.isRemote && PlayerHelper.hasEditPermission(((ServerPlayer) player), x, y, z))
		{
			...
			{
				moze_intel.projecte.compat.PECompatHelper.fillCauldron(world, x, y, z, meta + 1);
	...
	public boolean onItemRightClick(EntityPlayer player, float partial_tick, boolean ctrl_is_down){
		...
		if (!world.isRemote)
		{
			...
			if (mop != null && mop.getEntityHit() == null)
			{
				...
				int j = mop.blockY;
				int k = mop.blockZ;
				Block hitBlock = world.getBlock(i, j, k);
				int meta = world.getBlockMetadata(i, j, k);

				// MITE has no Item.onItemUse hook, so the cauldron filling used
				// to live in a method that was never called. Handle it here:
				// right-clicking a cauldron adds one water level.
				if (hitBlock == Blocks.cauldron && meta < 3)
				{
					if (PlayerHelper.hasEditPermission(((ServerPlayer) player), i, j, k))
					{
						moze_intel.projecte.compat.PECompatHelper.fillCauldron(world, i, j, k, meta + 1);
	...
	private void placeWater(World world, EntityPlayer player, int i, int j, int k)
	{
		if (world.provider.isHellWorld)
		{
			...
			for (int l = 0; l < 8; ++l)
			{
				world.spawnParticle(EnumParticle.largesmoke, (double)i + Math.random(), (double)j + Math.random(), (double)k + Math.random(), 0.0D, 0.0D, 0.0D);
			}
		}
		else
		{
			PlayerHelper.checkedPlaceBlock(((ServerPlayer) player), i, j, k, Blocks.flowing_water, 0);
		}
	...
	public void updateInPedestal(World world, int x, int y, int z)
	{
		if (!world.isRemote && ProjectEConfig.evertidePedCooldown != -1)
		{
			...
			if (tile.getActivityCooldown() == 0)
			{
				...
				moze_intel.projecte.compat.PECompatHelper.setRainTime(world, i);
				moze_intel.projecte.compat.PECompatHelper.setThunderTime(world, i);
				moze_intel.projecte.compat.PECompatHelper.setRaining(world, true);
