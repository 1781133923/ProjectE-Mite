...
public class InterdictionTorch extends BlockTorch implements net.minecraft.ITileEntityProvider
{
	...
	public void randomDisplayTick(World world, int x, int y, int z, Random rand)
	{
		...
		if (l == 1)
			world.spawnParticle(EnumParticle.smoke, d0 - d4, d1 + d3, d2, 0.0D, 0.0D, 0.0D);
		else if (l == 2)
			world.spawnParticle(EnumParticle.smoke, d0 + d4, d1 + d3, d2, 0.0D, 0.0D, 0.0D);
		else if (l == 3)
			world.spawnParticle(EnumParticle.smoke, d0, d1 + d3, d2 - d4, 0.0D, 0.0D, 0.0D);
		else if (l == 4)
			world.spawnParticle(EnumParticle.smoke, d0, d1 + d3, d2 + d4, 0.0D, 0.0D, 0.0D);
		else
			world.spawnParticle(EnumParticle.smoke, d0, d1, d2, 0.0D, 0.0D, 0.0D);
	}
