...
public class TransmutationProps implements IExtendedEntityProperties
{
	...
	{
		moze_intel.projecte.compat.ExtendedProperties.register(player, PROP_NAME, new TransmutationProps(player));
	}
	...
	public static TransmutationProps getDataFor(EntityPlayer player)
	{
		...
		return ((TransmutationProps) props);
	}
	...
	{
		if (EMCMapper.emc.isEmpty())
		{
			// With FishModLoader the player entity joins (and syncs knowledge)
			// BEFORE FMLServerStartingEvent runs EMCMapper.map(), and the map
			// is also cleared again on shutdown. Pruning while the map is empty
			// would delete every learned item from the save; only prune once
			// the EMC values actually exist.
			PELogger.logDebug("Skipping stale-knowledge prune: EMC map not ready");
			return;
		}
	...
	public void readFromPacket(NBTTagCompound compound)
	{
		...
		NBTTagList list = compound.getTagList("knowledge");
		knowledge.clear();
		...
		NBTTagList list2 = compound.getTagList("inputlocks");
	...
	public void loadNBTData(NBTTagCompound compound)
	{
		...
		// Always start from an empty list: this object may have been reused
		// from a previous world (in-memory props survive world switches), so
		// loading must not keep stale knowledge from another save.
		knowledge.clear();
		NBTTagList list = properties.getTagList("knowledge");
		for (int i = 0; i < list.tagCount(); i++)
		{
			ItemStack item = ItemStack.loadItemStackFromNBT(((NBTTagCompound) list.tagAt(i)));
			if (item != null)
		...
		NBTTagList list2 = properties.getTagList("inputlock");
		inputLocks = ItemHelper.copyIndexedNBTToArray(list2, new ItemStack[9]);
	}
