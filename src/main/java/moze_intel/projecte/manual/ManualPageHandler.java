...
public class ManualPageHandler
{
    ...
    public static void init()
    {
        ...
        if (mc == null)
        {
            // FML entrypoints run before the client instance exists; defer until launch completes.
            moddedmite.rustedironcore.api.event.Handlers.Initialization.register(new moddedmite.rustedironcore.api.event.listener.IInitializationListener()
            {
                @Override
                public void onClientStarted(net.minecraft.Minecraft client)
                {
        ...
        net.minecraft.ResourceManager resourceManager = mc.getResourceManager();
        if (resourceManager instanceof net.minecraft.ReloadableResourceManager)
        {
            ((net.minecraft.ReloadableResourceManager) resourceManager).registerReloadListener(new net.minecraft.ResourceManagerReloadListener()
            {
                @Override
                public void onResourceManagerReload(net.minecraft.ResourceManager p_110549_1_)
                {
    ...
    {
        addItem(net.minecraft.Item.getItem(block), category);
    }
    ...
    private static List<ItemStack> getSubItems(Item i)
    {
        ...
        // MITE's ItemBlock.getSubItems forwards this id straight into the
        // generated ItemStacks. Passing 0 produced ItemStacks with itemID 0
        // (empty slot in MITE's itemsList, IDs start at 256), which crashed
        // while building the manual pages for subtype blocks like the fuel
        // block. Use the item's real id.
        i.getSubItems(i.itemID, null, list);
        return list;
    }
}
