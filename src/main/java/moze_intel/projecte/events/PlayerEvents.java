...
public class PlayerEvents
{
	...
	{
		if (!event.entity.worldObj.isRemote && event.entity instanceof ServerPlayer)
		{
	...
	public void onConstruct(EntityEvent.EntityConstructing evt)
	{
		if (evt.entity instanceof EntityPlayer && !(evt.entity instanceof FakePlayer))
		{
			...
			// Only register fresh props when none exist for this player yet.
			// On death MITE constructs a new player entity (same name) before
			// EntityPlayer.clonePlayer() copies the old entity's state; if we
			// replaced the props here the clone event below would copy from an
			// empty object and transmutation knowledge/EMC + bag contents
			// would be lost on respawn.
			EntityPlayer player = ((EntityPlayer) evt.entity);
			if (moze_intel.projecte.compat.ExtendedProperties.get(player, TransmutationProps.PROP_NAME) == null)
			{
				TransmutationProps.register(player);
			}
			if (moze_intel.projecte.compat.ExtendedProperties.get(player, AlchBagProps.PROP_NAME) == null)
			{
	...
	public void onHighAlchemistJoin(cpw.mods.fml.common.gameevent.PlayerEvent.PlayerLoggedInEvent evt)
	{
		if (PECore.uuids.contains((evt.player.getUniqueID().toString())))
		{
			...
			MinecraftServer.getServer().getConfigurationManager().sendChatMsg(net.minecraft.ChatMessageComponent.createFromText(prior.appendSibling(playername).appendSibling(latter).getFormattedText())); // Sends to all everywhere, not just same world like before.
		}
	}
	...
	{
		PlayerChecks.onPlayerChangeDimension((ServerPlayer) event.player);
	}
	...
	public void pickupItem(EntityItemPickupEvent event)
	{
		...
		else
		{
			...
			ItemStack[] inv = AlchemicalBags.get(player, (byte) bag.getItemSubtype());
			...
			if (ItemHelper.hasSpace(inv, event.item.getEntityItem()))
			{
				...
				AlchemicalBags.set(player, (byte) bag.getItemSubtype(), inv);
				AlchemicalBags.syncPartial(player, bag.getItemSubtype());
