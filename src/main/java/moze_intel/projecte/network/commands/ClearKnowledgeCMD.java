...
public class ClearKnowledgeCMD extends ProjectEBaseCMD
{
	...
	public void processCommand(ICommandSender sender, String[] params) 
	{
		if (params.length == 0)
		{
			...
			{
				Transmutation.clearKnowledge(((ServerPlayer) sender));
				PacketHandler.sendTo(new KnowledgeClearPKT(), (ServerPlayer) sender);
		...
		else
		{
			for (Object obj : sender.getEntityWorld().playerEntities)
			{
				...
				if (player.getCommandSenderName().equalsIgnoreCase(params[0]))
				{
					...
					PacketHandler.sendTo(new KnowledgeClearPKT(), (ServerPlayer) player);
					...
					{
						moze_intel.projecte.compat.PEChatHelper.send(player, ChatHelper.modifyColor(new ChatComponentTranslation("pe.command.clearknowledge.notify", sender.getCommandSenderName()), EnumChatFormatting.RED));
					}
